...
class InceptionV3(nn.Module):
    ...
    def __call__(self, x, train=True, rng=jax.random.PRNGKey(0)):
        ...
        x = InceptionB(
            params_dict=utils.get(self.params_dict, "Mixed_6a"), dtype=self.dtype
        )(x, train)
        x = InceptionC(
            channels_7x7=128,
        ...
        x = InceptionD(
            params_dict=utils.get(self.params_dict, "Mixed_7a"), dtype=self.dtype
        )(x, train)
        x = InceptionE(
            avg_pool,
            params_dict=utils.get(self.params_dict, "Mixed_7b"),
            dtype=self.dtype,
        )(x, train)
