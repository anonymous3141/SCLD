...
def compute_statistics_with_mmap(
    path, mmap_filname, params, apply_fn, batch_size=1, img_size=None
):
    ...
    assert (
        directory_iterator.samples > 0
    ), "No images found. Make sure your images are within a subdirectory."
    ...
    mm = np.memmap(
        mmap_filname, dtype=dtype, mode="w+", shape=(num_activations, activation_dim)
    )
