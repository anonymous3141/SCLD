...
class TimeEncoder(nn.Module):
    ...
    def setup(self):
        self.timestep_phase = self.param(
            "timestep_phase", nn.initializers.normal(stddev=1.0), (1, self.num_hid)
        )
...
class LangevinNetwork(nn.Module):
    ...
    def setup(self):
        ...
        self.lgv_net = LangevinScaleNet(
            num_layers=self.lgv_num_layers,
            num_hid=self.lgv_num_hid,
            lgv_per_dim=self.lgv_per_dim,
            parent=self,
        )

        self.state_time_coder = TimeEncoder(self.state_time_num_hid)
        self.state_time_net = StateTimeEncoder(
            num_layers=self.state_time_num_layers,
            num_hid=self.state_time_num_hid,
            parent=self,
        )
