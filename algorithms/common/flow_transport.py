...
class GeometricAnnealingSchedule(object):
    ...
    def __call__(self, step: int, samples: Samples):
...
def get_delta_no_flow(
    samples: Samples, log_density: LogDensityByStep, step: int
) -> Array:
...
def transport_free_energy_estimator(
    ...
    if not use_path_gradient:
        ...
        deltas = get_delta_path_grad(
            samples, flow_apply, inv_flow_apply, flow_params, log_density, step
        )
...
def get_log_normalizer_increment_no_flow_verse(
    deltas: Array, log_weights: Array
) -> Array:
    assert_equal_shape([deltas, log_weights])
    normalized_log_weights = jax.nn.log_softmax(log_weights)
    total_terms = normalized_log_weights + deltas
...
def get_log_normalizer_increment_no_flow_craft(
    deltas: Array, log_weights: Array
) -> Array:
    assert_equal_shape([deltas, log_weights])
    normalized_log_weights = jax.nn.log_softmax(log_weights)
...
def get_log_normalizer_increment(
    ...
    if reverse:
        deltas = -get_delta_reverse(samples, flow_apply, flow_params, log_density, step)
...
def get_log_normalizer_increment_craft(
    samples: Samples,
    log_weights: Array,
    flow_apply: FlowApply,
    flow_params: FlowParams,
...
def update_samples_log_weights(
    ...
    reverse=False,
) -> Tuple[Array, Array, AcceptanceTuple]:
    """Update samples and log weights once the flow has been learnt."""
    transformed_samples, _ = flow_apply(flow_params, samples)
    assert_trees_all_equal_shapes(transformed_samples, samples)
    log_weights_new = reweight(
        log_weights, samples, flow_apply, flow_params, log_density, step, reverse
    )
