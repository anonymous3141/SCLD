...
import optax
from algorithms.fab.buffer.prioritised_buffer import (PrioritisedBuffer,
                                                      build_prioritised_buffer)
from algorithms.fab.flow import FlowDistConfig, build_flow
from algorithms.fab.sampling import (build_blackjax_hmc, build_metropolis,
                                     build_smc, default_point_is_valid_fn,
                                     point_is_valid_if_in_bounds_fn,
                                     simple_resampling)
from algorithms.fab.train import (TrainStateNoBuffer, TrainStateWithBuffer,
...
def setup_plotter(
    ...
    def get_data_for_plotting(
        ...
        point, log_w, smc_state, smc_info = smc.step(
            x0, state.smc_state, log_q_fn, target.log_prob
        )
        x_smc = point.x
...
def setup_fab_config(cfg, target) -> TrainConfig:
    ...
    # Setup buffer.
    buffer_max_length = (
        alg_cfg.training.batch_size * alg_cfg.buffer.buffer_max_length_in_batches
    )
    buffer_min_length = (
        alg_cfg.training.batch_size * alg_cfg.buffer.buffer_min_length_in_batches
    )
    ...
    n_iter_warmup = (
        opt_cfg.pop("warmup_n_epoch") * alg_cfg.buffer.n_updates_per_smc_forward_pass
    )
    n_iter_total = (
        alg_cfg.training.n_epoch * alg_cfg.buffer.n_updates_per_smc_forward_pass
    )
    ...
    train_config = TrainConfig(
        ...
        batch_size=alg_cfg.training.batch_size,
        seed=cfg.seed,
        eval_freq=alg_cfg.training.n_epoch // 100,
        init_state=init,
        update=step,
