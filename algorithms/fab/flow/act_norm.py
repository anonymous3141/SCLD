...
class UnconditionalAffine(distrax.Bijector):
    ...
    def __init__(
        self,
        get_params: Callable[[], Tuple[chex.Array, chex.Array]],
        restrict_scaling: bool = True,
    ) -> None:
        super().__init__(
            event_ndims_in=1, is_constant_jacobian=True, is_constant_log_det=True
        )
    ...
    def forward_and_log_det(self, x: chex.Array) -> Tuple[chex.Array, chex.Array]:
        ...
        bijector = distrax.Block(
            distrax.ScalarAffine(shift=shift, scale=scale), ndims=1
        )
...
def build_act_norm_layer(
    ...
    class GetScaleShift(nn.Module):
        ...
        def __call__(self) -> Tuple[chex.Array, chex.Array]:
            scale_logit = self.param(
                "scale_logit", nn.initializers.zeros_init(), (dim,)
            )
