...
from algorithms.langevin_diffusion.cmcd import \
    per_sample_elbo as cmcd_per_sample_elbo
...
from algorithms.langevin_diffusion.od_langevin import \
    per_sample_elbo as od_per_sample_elbo
from algorithms.langevin_diffusion.ud_langevin import \
    per_sample_elbo as ud_per_sample_elbo
...
def eval_langevin(
    ...
    def short_eval(params_flat, key):
        ...
        log_ratio, (samples, _) = jax.vmap(
            per_sample_elbo, in_axes=(0, None, None, None, None)
        ...
        if cfg.moving_average.use_ma:
            logger.update(
                moving_averages(logger, window_size=cfg.moving_average.window_size)
            )
...
def load_and_eval_langevin(
    ...
    params_train = pickle.load(
        open(project_path(f"models/{model_path}_train.pkl"), "rb")
    )
    params_notrain = pickle.load(
        open(project_path(f"models/{model_path}_notrain.pkl"), "rb")
    )
