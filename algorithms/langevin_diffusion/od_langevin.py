...
from algorithms.langevin_diffusion.ld_utils import (log_prob_kernel,
                                                    sample_kernel)
...
def evolve_overdamped_langevin(
    ...
    def evolve(aux, i):
        ...
        # Forward kernel
        fk_mean = x - params["eps"] * jax.grad(U)(
            x, beta
        ...
        if alg == "ULA":  # Unadjusted Langevin Annealing (ULA)
            ...
            bk_mean = (
                x_new
                - params["eps"] * jax.grad(U)(x_new, beta)
                + params["eps"]
                * apply_fun_approx_network(params["approx_network"], x_new, i)
            )
...
def evolve_overdamped_langevin_reverse(
    ...
    def evolve(aux, i):
        ...
        if alg == "ULA":  # Unadjusted Langevin Annealing (ULA)
            ...
            bk_mean = (
                x
                - params["eps"] * jax.grad(U)(x, beta)
                + params["eps"]
                * apply_fun_approx_network(params["approx_network"], x, i)
            )
        ...
        # Forward kernel
        fk_mean = x_new - params["eps"] * jax.grad(U)(
            x_new, beta
        )  # - because it is gradient of U = -log \pi
    ...
    aux = (x, 0, rng_key_gen)
    aux, _ = jax.lax.scan(
        evolve, aux, jnp.arange(num_temps)[::-1]
...
def per_sample_eubo(
    seed, params_flat, unflatten, params_fixed, log_prob, target_samples
):
    params_train, params_notrain = unflatten(params_flat)
    params_notrain = jax.lax.stop_gradient(params_notrain)
    params = {**params_train, **params_notrain}  # Gets all parameters in single place
