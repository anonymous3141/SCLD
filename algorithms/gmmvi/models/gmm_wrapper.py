...
def setup_gmm_wrapper(
    gmm: GMM,
    INITIAL_STEPSIZE,
    INITIAL_REGULARIZER,
    MAX_REWARD_HISTORY_LENGTH,
    INITIAL_LAST_ETA=-1,
):
    def init_gmm_wrapper_state(gmm_state: GMMState):
        return GMMWrapperState(
            ...
            reward_history=jnp.finfo(jnp.float32).min
            * jnp.ones((gmm_state.num_components, MAX_REWARD_HISTORY_LENGTH)),
            weight_history=jnp.finfo(jnp.float32).min
            * jnp.ones((gmm_state.num_components, MAX_REWARD_HISTORY_LENGTH)),
    ...
    def add_component(
        ...
        initial_weight: jnp.float32,
        initial_mean: chex.Array,
        initial_cov: chex.Array,
        adding_threshold: chex.Array,
    ):

        return GMMWrapperState(
            gmm_state=gmm.add_component(
                gmm_wrapper_state.gmm_state, initial_weight, initial_mean, initial_cov
            ),
            l2_regularizers=jnp.concatenate(
                (gmm_wrapper_state.l2_regularizers, jnp.ones(1) * INITIAL_REGULARIZER),
                axis=0,
            ),
            last_log_etas=jnp.concatenate(
                (gmm_wrapper_state.last_log_etas, jnp.ones(1) * INITIAL_LAST_ETA),
                axis=0,
            ),
            num_received_updates=jnp.concatenate(
                (gmm_wrapper_state.num_received_updates, jnp.zeros(1)), axis=0
            ),
            stepsizes=jnp.concatenate(
                (gmm_wrapper_state.stepsizes, jnp.ones(1) * INITIAL_STEPSIZE), axis=0
            ),
            reward_history=jnp.concatenate(
                (
                    gmm_wrapper_state.reward_history,
                    jnp.ones((1, MAX_REWARD_HISTORY_LENGTH))
            ...
            weight_history=jnp.concatenate(
                (
                    gmm_wrapper_state.weight_history,
                    jnp.ones((1, MAX_REWARD_HISTORY_LENGTH)) * initial_weight,
                ),
                axis=0,
            ),
            unique_component_ids=jnp.concatenate(
                (
                    gmm_wrapper_state.unique_component_ids,
                    jnp.ones(1, dtype=jnp.int32) * gmm_wrapper_state.max_component_id,
                ),
                axis=0,
            ),
            max_component_id=gmm_wrapper_state.max_component_id + 1,
            adding_thresholds=jnp.concatenate(
                (gmm_wrapper_state.adding_thresholds, adding_threshold), axis=0
            ),
        )
    ...
    def remove_component(gmm_wrapper_state: GMMWrapperState, idx: int):
        return GMMWrapperState(
            ...
            unique_component_ids=jnp.concatenate(
                (
                    gmm_wrapper_state.unique_component_ids[:idx],
                    gmm_wrapper_state.unique_component_ids[idx + 1 :],
                ),
                axis=0,
            ),
            l2_regularizers=jnp.concatenate(
                (
                    gmm_wrapper_state.l2_regularizers[:idx],
                    gmm_wrapper_state.l2_regularizers[idx + 1 :],
                ),
                axis=0,
            ),
            last_log_etas=jnp.concatenate(
                (
                    gmm_wrapper_state.last_log_etas[:idx],
                    gmm_wrapper_state.last_log_etas[idx + 1 :],
                ),
                axis=0,
            ),
            num_received_updates=jnp.concatenate(
                (
                    gmm_wrapper_state.num_received_updates[:idx],
                    gmm_wrapper_state.num_received_updates[idx + 1 :],
                ),
                axis=0,
            ),
            stepsizes=jnp.concatenate(
                (
                    gmm_wrapper_state.stepsizes[:idx],
                    gmm_wrapper_state.stepsizes[idx + 1 :],
                ),
                axis=0,
            ),
            reward_history=jnp.concatenate(
                (
                    gmm_wrapper_state.reward_history[:idx],
                    gmm_wrapper_state.reward_history[idx + 1 :],
                ),
                axis=0,
            ),
            weight_history=jnp.concatenate(
                (
                    gmm_wrapper_state.weight_history[:idx],
                    gmm_wrapper_state.weight_history[idx + 1 :],
                ),
                axis=0,
            ),
            adding_thresholds=jnp.concatenate(
                (
                    gmm_wrapper_state.adding_thresholds[:idx],
                    gmm_wrapper_state.adding_thresholds[idx + 1 :],
                ),
                axis=0,
            ),
        )
    ...
    def update_weights(gmm_wrapper_state: GMMWrapperState, new_log_weights: chex.Array):
        return GMMWrapperState(
            gmm_state=gmm.replace_weights(gmm_wrapper_state.gmm_state, new_log_weights),
            weight_history=jnp.concatenate(
                (
                    gmm_wrapper_state.weight_history[:, 1:],
                    jnp.expand_dims(
                        jnp.exp(gmm_wrapper_state.gmm_state.log_weights), 1
                    ),
                ),
                axis=1,
            ),
    ...
    def update_rewards(gmm_wrapper_state: GMMWrapperState, rewards: chex.Array):
        return GMMWrapperState(
            gmm_state=gmm_wrapper_state.gmm_state,
            l2_regularizers=gmm_wrapper_state.l2_regularizers,
            last_log_etas=gmm_wrapper_state.last_log_etas,
            num_received_updates=gmm_wrapper_state.num_received_updates,
            stepsizes=gmm_wrapper_state.stepsizes,
            reward_history=jnp.concatenate(
                (gmm_wrapper_state.reward_history[:, 1:], jnp.expand_dims(rewards, 1)),
                axis=1,
            ),
    ...
    return GMMWrapper(
        init_gmm_wrapper_state=init_gmm_wrapper_state,
