...
def get_default_algorithm_config(algorithm_id):
    ...
    MODULE_CONF_PATH = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "module_configs"
    )
    ...
    LETTER_TO_PATH = {
        ...
        "T": os.path.join(
            MODULE_CONF_PATH, "ng_based_component_updater", "trust-region.yml"
        ),
        "F": os.path.join(
            MODULE_CONF_PATH, "component_stepsize_adaptation", "fixed.yml"
        ),
        "D": os.path.join(
            MODULE_CONF_PATH, "component_stepsize_adaptation", "decaying.yml"
        ),
        "R": os.path.join(
            MODULE_CONF_PATH, "component_stepsize_adaptation", "improvement-based.yml"
        ),
        ...
        "G": os.path.join(
            MODULE_CONF_PATH, "weight_stepsize_adaptation", "decaying.yml"
        ),
        "N": os.path.join(
            MODULE_CONF_PATH, "weight_stepsize_adaptation", "improvement-based.yml"
        ),
    }

    merged_config = dict()
    [
        merge(merged_config, load_yaml(LETTER_TO_PATH[letter.upper()]))
        for letter in algorithm_id
    ]
