...
from algorithms.gmmvi.configs import (get_default_algorithm_config,
                                      update_config)
...
from algorithms.gmmvi.optimization.gmmvi_modules.component_adaptation import (
    ComponentAdaptationState, setup_fixed_component_adaptation,
...
from algorithms.gmmvi.optimization.gmmvi_modules.ng_estimator import (
    NgEstimatorState, setup_more_ng_estimator, setup_stein_ng_estimator)
from algorithms.gmmvi.optimization.gmmvi_modules.sample_selector import (
    setup_fixed_sample_selector, setup_lin_sample_selector,
    setup_vips_sample_selector)
from algorithms.gmmvi.optimization.gmmvi_modules.weight_stepsize_adaptation import (
    WeightStepsizeAdaptationState, setup_decaying_weight_stepsize_adaptation,
...
from algorithms.gmmvi.optimization.gmmvi_modules.weight_updater import (
    setup_direct_weight_updater, setup_trust_region_based_weight_updater)
from algorithms.gmmvi.optimization.sample_db import (SampleDBState,
                                                     setup_sampledb)
...
def setup_gmmvi(config, target, seed):
    ...
    config = update_config(
        update_config(get_default_algorithm_config(config.algorithm.algorithm), config),
        config.algorithm,
    )
    ...
    if config["ng_estimator_type"] == "Stein":
        ng_estimator = setup_stein_ng_estimator(
            model,
            dim,
            config["model_initialization"]["use_diagonal_covs"],
    ...
    #     ng_estimator_state = ng_estimator.init_ng_estimator_state(quad_regression_state)
        ...
        raise ValueError(
            f"config['ng_estimator_type'] is '{config['ng_estimator_type']}' "
            f"which is an unknown type"
        )

    # setup NgBasedComponentUpdater
    if config["ng_based_updater_type"] == "trust-region":
        ng_based_component_updater = setup_kl_constrained_ng_based_component_updater(
            model,
            dim,
            config["model_initialization"]["use_diagonal_covs"],
            config["temperature"],
    ...
    if config["num_component_adapter_type"] == "adaptive":
        component_adapter = setup_vips_component_adaptation(
            sample_db,
            model,
            target_log_prob,
            dim,
    ...
    if config["component_stepsize_adapter_type"] == "improvement-based":
        component_stepsize_adapter = setup_improvement_based_stepsize_adaptation(
            config["component_stepsize_adapter_config"]["min_stepsize"],
        ...
        component_stepsize_adapter = setup_decaying_component_stepsize_adaptation(
            config["component_stepsize_adapter_config"]["initial_stepsize"],
    ...
    if config["sample_selector_type"] == "fixed":
        sample_selector = setup_fixed_sample_selector(
            sample_db,
            model,
            target_log_prob,
            config["sample_selector_config"]["desired_samples_per_component"],
        ...
        sample_selector = setup_lin_sample_selector(
            sample_db,
            model,
            target_log_prob,
            config["sample_selector_config"]["desired_samples_per_component"],
        ...
        sample_selector = setup_vips_sample_selector(
            sample_db,
            model,
            target_log_prob,
            config["sample_selector_config"]["desired_samples_per_component"],
    ...
    if config["weight_stepsize_adapter_type"] == "fixed":
        ...
        weight_stepsize_adapter_state = (
            weight_stepsize_adapter.init_weight_stepsize_adaptation(
                config["weight_stepsize_adapter_config"]["initial_stepsize"]
            )
        )
    elif config["weight_stepsize_adapter_type"] == "decaying":
        weight_stepsize_adapter = setup_decaying_weight_stepsize_adaptation(
            config["weight_stepsize_adapter_config"]["initial_stepsize"],
        ...
        weight_stepsize_adapter = setup_improvement_based_weight_stepsize_adaptation(
            config["weight_stepsize_adapter_config"]["min_stepsize"],
    ...
    def train_iter(train_state: TrainState, key: chex.Array):
        ...
        (
            ...
            train_state.model_state, train_state.sample_db_state, subkey
        )
        new_component_stepsizes = component_stepsize_adapter.update_stepsize(
            train_state.model_state
        )
        new_model_state = model.update_stepsizes(
            train_state.model_state, new_component_stepsizes
        )
        expected_hessian_neg, expected_grad_neg = (
            ng_estimator.get_expected_hessian_and_grad(
                new_model_state,
                samples,
                mapping,
                sample_dist_densities,
        ...
        new_model_state = ng_based_component_updater.apply_NG_update(
            new_model_state,
        ...
        new_weight_stepsize_adapter_state = weight_stepsize_adapter.update_stepsize(
            train_state.weight_stepsize_adapter_state, new_model_state
        )
        new_model_state = weight_updater.update_weights(
            new_model_state,
            samples,
            sample_dist_densities,
            target_lnpdfs,
        ...
        new_model_state, new_component_adapter_state, new_sample_db_state = (
            component_adapter.adapt_number_of_components(
    ...
    def eval(seed: chex.Array, train_state: TrainState, target_samples=None):
        samples = model.sample(
            train_state.model_state.gmm_state, seed, config["eval_samples"]
        )[0]
        log_prob_model = jax.vmap(model.log_density, in_axes=(None, 0))(
            train_state.model_state.gmm_state, samples
        )
        log_prob_target = jax.vmap(target.log_prob)(samples)
        log_ratio = log_prob_target - log_prob_model

        if target_samples is not None:
            fwd_log_prob_model = jax.vmap(model.log_density, in_axes=(None, 0))(
                train_state.model_state.gmm_state, target_samples
            )
    ...
    return GMMVI(
        initial_train_state=initial_train_state, train_iter=train_iter, eval=eval
    )
