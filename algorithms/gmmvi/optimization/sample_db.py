...
def setup_sampledb(
    DIM, KEEP_SAMPLES, MAX_SAMPLES, DIAGONAL_COVS, DESIRED_SAMPLES_PER_COMPONENT
) -> SampleDB:
    ...
    def add_samples(
        ...
        def _remove_every_nth_sample(sampledb_state: SampleDBState, N: int):
            ...
            return SampleDBState(
                num_samples_written=sampledb_state.num_samples_written,
        ...
        if (
            ...
            and jnp.shape(new_samples)[0] + jnp.shape(sampledb_state.samples)[0]
            > MAX_SAMPLES
        ):
            sampledb_state = _remove_every_nth_sample(sampledb_state, 2)

        num_samples_written = (
            sampledb_state.num_samples_written + jnp.shape(new_samples)[0]
        )
        if KEEP_SAMPLES:
            mapping = jnp.concatenate(
                (
                    sampledb_state.mapping,
                    new_mapping + jnp.shape(sampledb_state.chols)[0],
                )
            )
            ...
            target_lnpdfs = jnp.concatenate(
                (sampledb_state.target_lnpdfs, new_target_lnpdfs)
            )
            target_grads = jnp.concatenate(
                (sampledb_state.target_grads, new_target_grads)
            )

            if DIAGONAL_COVS:
                inv_chols = jnp.concatenate((sampledb_state.inv_chols, 1.0 / new_chols))
            else:
                inv_chols = jnp.concatenate(
                    (sampledb_state.inv_chols, jnp.linalg.inv(new_chols))
                )
        else:
            mapping = new_mapping
            means = new_means
    ...
    def get_random_sample(sample_db_state: SampleDBState, N: int, seed: chex.PRNGKey):
        chosen_indices = jax.random.permutation(
            seed, jnp.arange(jnp.shape(sample_db_state.samples)[0]), independent=True
        )[:N]
        # chosen_indices = Randomness.get_next_random()
        return (
            sample_db_state.samples[chosen_indices],
            sample_db_state.target_lnpdfs[chosen_indices],
        )

    def _gaussian_log_pdf(mean, chol, inv_chol, x):
        if DIAGONAL_COVS:
            constant_part = -0.5 * DIM * jnp.log(2 * jnp.pi) - jnp.sum(jnp.log(chol))
            return constant_part - 0.5 * jnp.sum(
                jnp.square(
                    jnp.expand_dims(inv_chol, 1)
                    * jnp.transpose(jnp.expand_dims(mean, 0) - x)
                ),
                axis=0,
            )
        else:
            constant_part = -0.5 * DIM * jnp.log(2 * jnp.pi) - jnp.sum(
                jnp.log(jnp.diag(chol))
            )
            return constant_part - 0.5 * jnp.sum(
                jnp.square(inv_chol @ jnp.transpose(mean - x)), axis=0
            )
    ...
    def get_newest_samples_deprecated(sampledb_state: SampleDBState, N):
        def _compute_log_pdfs(sampledb_state, component_id, sample):
            return jax.lax.cond(
                ...
                lambda: _gaussian_log_pdf(
                    sampledb_state.means[component_id],
                    sampledb_state.chols[component_id],
                    sampledb_state.inv_chols[component_id],
                    sample,
                ),
            )
        ...
        if jnp.shape(sampledb_state.samples)[0] == 0 or N == 0:
            ...
            active_sample_index = jnp.maximum(
                0, jnp.shape(sampledb_state.samples)[0] - N
            )
            ...
            def compute_background_pdf():
                active_components, count = jnp.unique(
                    active_mapping,
                    return_counts=True,
                    size=sampledb_state.means.shape[0],
                    fill_value=-1,
                )
                weights = count / jnp.sum(count)
                return jax.nn.logsumexp(
                    jax.vmap(_compute_log_pdfs, in_axes=(None, 0, None))(
                        sampledb_state, active_components, active_sample
                    )
                    + jnp.expand_dims(jnp.log(weights), 1),
                    axis=0,
                )
    ...
    def get_newest_samples(sampledb_state: SampleDBState, N):
        ...
        if jnp.shape(sampledb_state.samples)[0] == 0 or N == 0:
            ...
            active_components = jnp.arange(
                jnp.maximum(sampledb_state.means.shape[0] - num_active_comps, 0),
                sampledb_state.means.shape[0],
            )
            weights_test = (
                jnp.ones_like(active_components) / jnp.shape(active_components)[0]
            )

            def compute_background_pdf():
                log_pdfs = jax.vmap(_compute_log_pdfs, in_axes=(None, 0, None))(
                    sampledb_state, active_components, active_sample
                ) + jnp.expand_dims(jnp.log(weights_test), 1)
    ...
    def update_num_samples_written(sample_db_state: SampleDBState, num_samples_written):
        return SampleDBState(
            samples=sample_db_state.samples,
    ...
    return SampleDB(
        init_sampleDB_state=init_sample_db_state,
