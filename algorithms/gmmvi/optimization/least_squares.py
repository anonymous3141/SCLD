...
def setup_quad_regression(DIM):
    def init_quad_reg_state():
        return QuadRegressionState(
            ...
            triu_idx_const=jnp.array(
                jnp.transpose(
                    jnp.stack(jnp.where(jnp.triu(jnp.ones([DIM, DIM], jnp.bool_))))
                )
            ),
        )

    def fit_quadratic(
        quad_reg_state: QuadRegressionState,
        regularizer: float,
        num_samples: int,
        inputs: chex.Array,
        outputs: chex.Array,
        weights: chex.Array = None,
        sample_mean: chex.Array = None,
        sample_chol_cov: chex.Array = None,
        ...
        def _fit(
            ...
            def _feature_fn(
                quad_reg_state, num_samples: int, x: chex.Array
            ) -> chex.Array:
                linear_features = x
                ...
                for i in range(quad_reg_state.dim):
                    quad_features = jnp.concatenate(
                        (quad_features, jnp.expand_dims(x[:, i], axis=1) * x[:, i:]),
                        axis=1,
                    )

                # stack quadratic features, linear features and constant features
                features = jnp.concatenate(
                    (quad_features, linear_features, constant_feature), axis=1
                )
                return features
        ...
        params = _fit(
            quad_reg_state, regularizer, num_samples, inputs, outputs, weights
        )

        qt = jnp.zeros((DIM, DIM))
        qt[quad_reg_state.triu_idx_const[:, 0], quad_reg_state.triu_idx_const[:, 1]] = (
            params[-(DIM + 1)]
        )
        ...
        if whitening and sample_mean is not None and sample_chol_cov is not None:
            quad_term = (
                jnp.transpose(inv_samples_chol_cov) @ quad_term @ inv_samples_chol_cov
            )
    ...
    return QuadRegression(
        init_quad_reg_state=init_quad_reg_state, fit_quadratic=fit_quadratic
    )
