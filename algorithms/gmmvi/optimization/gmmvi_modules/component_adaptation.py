...
def setup_fixed_component_adaptation():
    def init_fixed_component_adaptation_state():
        return ComponentAdaptationState(
            num_calls_to_add_heuristic=None,
            reward_improvements=None,
        )

    def adapt_number_of_components_fixed(
    ...
    return ComponentAdaptation(
        init_component_adaptation=init_fixed_component_adaptation_state,
        adapt_number_of_components=adapt_number_of_components_fixed,
    )


def setup_vips_component_adaptation(
    sample_db: SampleDB,
    gmm_wrapper: GMMWrapper,
    target_log_prob,
    DIM: int,
    PRIOR_MEAN: Union[float, chex.Array],
    PRIOR_COV: Union[float, chex.Array],
    DIAGONAL_COVS: bool,
    DEL_ITERS: int,
    ADD_ITERS: int,
    MAX_COMPONENTS: int,
    THRESHOLD_FOR_ADD_HEURISTIC: chex.Array,
    MIN_WEIGHT_FOR_DEL_HEURISTIC: float,
    ...
    GAUSSIAN = dist.Normal(0, jnp.array(DEL_ITERS / 8.0, jnp.float32))
    KERNEL = jnp.exp(
        GAUSSIAN.log_prob(
            jnp.arange(start=-FILTER_DELAY, stop=FILTER_DELAY, dtype=jnp.float32)
        )
    )
    ...
    def _select_samples_for_adding_heuristic(
        ...
        samples, target_lnpdfs = sample_db.get_random_sample(
            sample_db_state, NUM_DATABASE_SAMPLES, subkey
        )
        prior_samples = jnp.zeros((0, DIM), jnp.float32)
        key, subkey = jax.random.split(key)
        if NUM_PRIOR_SAMPLES > 0:
            prior_samples = _sample_from_prior(NUM_PRIOR_SAMPLES, subkey)
            sample_db_state = sample_db.update_num_samples_written(
                sample_db_state,
    ...
    def _add_at_best_location(
        ...
        def _get_prior_entropy(chol_cov):
            avg_entropy = 0.5 * DIM * (jnp.log(2 * jnp.pi) + 1) + jnp.sum(
                jnp.log(chol_cov)
            )
            return avg_entropy

        it = component_adaption_state.num_calls_to_add_heuristic % len(
            THRESHOLD_FOR_ADD_HEURISTIC
        )
        model_log_densities = jax.vmap(gmm_wrapper.log_density, in_axes=(None, 0))(
        ...
        if PRIOR_MEAN is not None and PRIOR_CHOL_COV is not None:
            des_entropy = gmm_wrapper.average_entropy(
                gmm_wrapper_state.gmm_state
            ) * a + _get_prior_entropy(PRIOR_CHOL_COV) * (1 - a)
        else:
            des_entropy = gmm_wrapper.average_entropy(gmm_wrapper_state.gmm_state)
        max_logdensity = jnp.max(model_log_densities)
        rewards = target_lnpdfs - jnp.maximum(
            max_logdensity - THRESHOLD_FOR_ADD_HEURISTIC[it], model_log_densities
        )
        ...
        return gmm_wrapper.add_component(
            gmm_wrapper_state,
            init_weight,
            new_mean,
            new_cov,
    ...
    def _add_new_component(
        ...
        (
            ...
            target_lnpdfs,
            prior_samples,
        ) = _select_samples_for_adding_heuristic(
        ...
        if NUM_PRIOR_SAMPLES > 0:
            ...
            target_lnpdfs = jnp.concatenate(
                (target_lnpdfs, target_log_prob(prior_samples)), 0
            )

        key, subkey = jax.random.split(key)
        return (
            _add_at_best_location(
                component_adaption_state,
                gmm_wrapper_state,
                samples,
                target_lnpdfs,
    ...
    def _delete_bad_components(
        ...
        current_smoothed_reward = jnp.mean(
            gmm_wrapper_state.reward_history[:, -jnp.size(KERNEL) :]
            * jnp.reshape(KERNEL, [1, -1]),
            axis=1,
        )
        old_smoothed_reward = jnp.mean(
            gmm_wrapper_state.reward_history[
                :, -jnp.size(KERNEL) - DEL_ITERS : -DEL_ITERS
            ]
            * jnp.reshape(KERNEL, [1, -1]),
            axis=1,
        )
        ...
        reward_improvements = (current_smoothed_reward - old_smoothed_reward) / jnp.abs(
            old_smoothed_reward
        )
        ...
        max_actual_weights = jnp.max(
            gmm_wrapper_state.weight_history[:, -jnp.size(KERNEL) - DEL_ITERS : -1],
            axis=1,
        )
        max_greedy_weights = jnp.max(
            jnp.exp(
                gmm_wrapper_state.reward_history[:, -jnp.size(KERNEL) - DEL_ITERS :]
                - jax.nn.logsumexp(
                    gmm_wrapper_state.reward_history[
                        :, -jnp.size(KERNEL) - DEL_ITERS :
                    ],
                    axis=0,
                    keepdims=True,
                )
        ...
        is_old_enough = (
            gmm_wrapper_state.reward_history[:, -DEL_ITERS]
            != -jnp.finfo(jnp.float32).max
        )
        is_bad = jnp.all(
            jnp.vstack((is_stagnating, is_low_weight, is_old_enough)), axis=0
        )
    ...
    def adapt_number_of_components(
        component_adaption_state: ComponentAdaptationState,
        sample_db_state: SampleDBState,
        gmm_wrapper_state: GMMWrapperState,
        iteration,
        ...
        if iteration > DEL_ITERS:
            gmm_wrapper_state, component_adaption_state = _delete_bad_components(
                component_adaption_state, gmm_wrapper_state
            )
        if iteration > 1 and iteration % ADD_ITERS == 0:
            if gmm_wrapper_state.gmm_state.num_components < MAX_COMPONENTS:
                gmm_wrapper_state, component_adaption_state, sample_db_state = (
                    _add_new_component(
                        component_adaption_state,
                        sample_db_state,
                        gmm_wrapper_state,
    ...
    return ComponentAdaptation(
        init_component_adaptation=init_vips_component_adaption_state,
