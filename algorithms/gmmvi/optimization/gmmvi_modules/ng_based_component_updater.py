...
def setup_direct_ng_based_component_updater(
    gmm_wrapper: GMMWrapper, DIM, DIAGONAL_COVS, TEMPERATURE: float, INITIAL_REGULARIZER
):
    ...
    def apply_ng_update(
        ...
        expected_hessians_neg: chex.Array,
        expected_gradients_neg: chex.Array,
        stepsizes: chex.Array,
    ):
        means = jnp.empty(
            (gmm_wrapper_state.gmm_state.num_components,), dtype=jnp.float32
        )
        chols = jnp.empty(
            (gmm_wrapper_state.gmm_state.num_components,), dtype=jnp.float32
        )
        ...
        for i in range(gmm_wrapper_state.gmm_state.num_components):
            ...
            delta_lin = expected_hessians_neg[i] @ jnp.expand_dims(
                old_mean, 1
            ) - jnp.expand_dims(expected_gradients_neg[i], 1)
        ...
        new_gmm_state = gmm_wrapper.replace_components(
            gmm_wrapper_state.gmm_state, means, chols
        )
    ...
    return NgBasedComponentUpdater(
        init_ng_based_component_updater_state=init_direct_ng_based_component_updater_state,
        apply_NG_update=apply_ng_update,
    )


def setup_ng_based_component_updater_iblr(
    gmm_wrapper: GMMWrapper, DIM, DIAGONAL_COVS, TEMPERATURE: float, INITIAL_REGULARIZER
):
        ...
        for i in range(gmm_wrapper_state.gmm_state.num_components):
            ...
            if DIAGONAL_COVS:
                correction_term = (
                    stepsizes[i]
                    / 2
                    * expected_hessians_neg[i]
                    * old_chol
                    * old_chol
                    * expected_hessians_neg[i]
                )
                old_inv_chol = 1.0 / old_chol
            ...
            if gmm_wrapper_state.num_received_updates[i] == 0:
                ...
                if DIAGONAL_COVS:
                    new_mean = (
                        old_mean + stepsizes[i] * old_chol * old_chol * delta_mean
                    )
                else:
                    new_mean = old_mean + jnp.squeeze(
                        ...
                        * old_chol
                        @ jnp.transpose(old_chol)
                        @ jnp.expand_dims(delta_mean, 1)
                    )

            new_precision = old_precision + stepsizes[i] * delta_precision
            if DIAGONAL_COVS:
                new_cov = 1.0 / new_precision
    ...
    return NgBasedComponentUpdater(
        init_ng_based_component_updater_state=init_ng_based_component_updater_iblr_state,
        apply_NG_update=apply_ng_update,
    )


def setup_kl_constrained_ng_based_component_updater(
    gmm_wrapper: GMMWrapper, DIM, DIAGONAL_COVS, TEMPERATURE: float, INITIAL_REGULARIZER
):
    ...
    def _kl(
        eta: jnp.float32,
        old_lin_term: chex.Array,
        old_precision: chex.Array,
        old_inv_chol: chex.Array,
        reward_lin: chex.Array,
        reward_quad: chex.Array,
        kl_const_part: jnp.float32,
        old_mean: chex.Array,
        eta_in_logspace: bool,
    ) -> [jnp.float32, chex.Array, chex.Array, chex.Array]:

        eta = jax.lax.cond(
            eta_in_logspace, lambda eta: jnp.exp(eta), lambda eta: eta, eta
        )
        ...
        if DIAGONAL_COVS:
            ...
            new_mean = 1.0 / new_precision * new_lin
            inv_chol_inv = 1.0 / chol_precision
            ...
            kl = 0.5 * (
                jnp.maximum(
                    0.0,
                    jnp.sum(
                        jnp.log(new_precision / old_precision)
            ...
            def false_fn():
                new_mean = jnp.reshape(
                    jax.scipy.linalg.cho_solve(
                        (chol_precision, True), jnp.expand_dims(new_lin, 1)
                    ),
                    [-1],
                )
                ...
                trace_term = jnp.square(
                    jnp.linalg.norm(inv_chol_inv @ jnp.transpose(old_inv_chol))
                )
                diff = old_mean - new_mean
                kl = 0.5 * (
                    kl_const_part
                    - new_logdet
                    + trace_term
                    + jnp.sum(jnp.square(jnp.dot(old_inv_chol, diff)))
                )

                return kl, new_mean, new_precision, inv_chol_inv

            kl, new_mean, new_precision, inv_chol_inv = jax.lax.cond(
                jnp.any(jnp.isnan(chol_precision)), true_fn, false_fn
            )

        return kl, new_mean, new_precision, inv_chol_inv

    # always in log_space
    def _bracketing_search(
        KL_BOUND: jnp.float32,
        lower_bound: jnp.float32,
        upper_bound: jnp.float32,
        old_lin_term: chex.Array,
        old_precision: chex.Array,
        old_inv_chol: chex.Array,
        reward_lin_term: chex.Array,
        reward_quad_term: chex.Array,
        kl_const_part: jnp.float32,
        old_mean: chex.Array,
        ...
        def cond_fn(carry):
            ...
            diff = jnp.minimum(
                jnp.exp(upper_bound) - jnp.exp(eta), jnp.exp(eta) - jnp.exp(lower_bound)
            )
            return (
                (it < 1000)
                & (diff >= 1e-1)
                & ((jnp.abs(KL_BOUND - kl) >= 1e-1 * KL_BOUND) | jnp.isnan(kl))
            )

        def body_fn(carry):
            it, lower_bound, upper_bound, eta, _, upper_bound_satisfies_constraint = (
                carry
            )
            kl = _kl(
                eta,
            ...
            def true_fn():
                ...
                return (
                    ...
                    eta,
                    kl,
                    upper_bound_satisfies_constraint,
                )
            ...
            def false_fn():
                (
                    new_upper_bound,
                    new_lower_bound,
                    new_upper_bound_satisfies_constraint,
                ) = jax.lax.cond(
                    KL_BOUND > kl,
            ...
            return jax.lax.cond(
                jnp.abs(KL_BOUND - kl) < 1e-1 * KL_BOUND, true_fn, false_fn
            )

        _, lower_bound, upper_bound, eta, kl, upper_bound_satisfies_constraint = (
            jax.lax.while_loop(
                cond_fn,
                body_fn,
                init_val=(
    ...
    def apply_ng_update(
        ...
        def _apply_gn_update_per_comp(
            old_chol, old_mean, last_eta, eps, reward_quad, expected_gradients_neg
        ):
            if DIAGONAL_COVS:
                ...
                reward_lin = (
                    jnp.squeeze(reward_quad @ jnp.expand_dims(old_mean, 1))
            ...
            lower_bound_const, upper_bound_const = jax.lax.cond(
                last_eta < 0,
                lambda last_eta: (jnp.array(-20.0), jnp.array(80.0)),
                lambda last_eta: (
                    jnp.maximum(0.0, jnp.log(last_eta) - 3),
                    jnp.log(last_eta) + 3,
                ),
                last_eta,
            )

            new_lower, new_upper = _bracketing_search(
                eps,
                lower_bound_const,
                upper_bound_const,
                old_lin_term,
            ...
            def true_lower_equals_upper():
                new_kl, new_mean, _, new_inv_chol_inv = _kl(
                    eta,
                    old_lin_term,
                    old_precision,
                    old_inv_chol,
                ...
                return jax.lax.cond(
                    (new_kl < jnp.finfo(jnp.float32).max)
                    & (~jnp.any(jnp.isnan(new_chol))),
                    ...
                    lambda: (
                        ...
                        old_chol,
                        -1.0,
                    ),  # values will be ignored anyway, if success is false
                )

            success, new_mean, new_chol, new_kl = jax.lax.cond(
                new_lower == new_upper,
                true_lower_equals_upper,
                lambda: (False, old_mean, old_chol, -1.0),
            )
            return jax.lax.cond(
                success,
                lambda: (new_chol, new_mean, new_kl, True, eta),
                lambda: (old_chol, old_mean, -1.0, False, -1.0),
            )

        chols, means, kls, successes, etas = jax.vmap(_apply_gn_update_per_comp)(
    ...
    return NgBasedComponentUpdater(
        init_ng_based_component_updater_state=init_kl_constrained_ng_based_component_updater,
        apply_NG_update=apply_ng_update,
    )
