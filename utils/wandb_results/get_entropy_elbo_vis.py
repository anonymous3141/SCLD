...
if __name__ == "__main__":
    ...
    if USE_MOVING_AVERAGE:
        ...
        elbo_mean = -moving_average(data_dict["local"][fields[1]], 5).mean(0)
    else:
        entropy_mean = data_dict["local"][fields[0]].mean(0)
        entropy_std = -data_dict["local"][fields[0]].std(0)
        elbo_mean = -data_dict["local"][fields[1]].mean(0)
    ...
    elbo_mean = elbo_mean[np.argsort(elbo_mean)]
    entropy_mean = entropy_mean[np.argsort(elbo_mean)[::-1]]
    idx = np.array([0, entropy_mean.shape[0] // 2, entropy_mean.shape[0] - 1])

    if alg in ["smc_f2", "aft_f2"]:
        ...
        plt.scatter(elbo_mean, entropy_mean)
    else:
        # plt.plot(elbo_mean[idx], entropy_mean[idx])
        plt.plot(elbo_mean, entropy_mean)
        plt.fill_between(
            elbo_mean, entropy_mean - entropy_std, entropy_mean + entropy_std, alpha=0.3
        )
