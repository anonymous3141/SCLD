...
class GaussianMixtureModel(Target):
    ...
    def visualise(
        ...
        if self.dim == 2:
            ...
            # Compute `log_prob` in every component.
            per_comp_pdf = self.mixture_distribution.components_distribution.log_prob(
                expanded
            )
            gating = jnp.exp(
                self.mixture_distribution.components_distribution.log_prob(expanded)
                - logsumexp(per_comp_pdf, 1)[:, None]
            )
            ...
            if samples is not None:
                plt.scatter(
                    samples[:300, 0], samples[:300, 1], c="r", alpha=0.6, marker="o"
                )
            ...
            plt.savefig(
                os.path.join(project_path("./figures/"), f"gmm.png"),
                bbox_inches="tight",
                pad_inches=0.1,
            )
