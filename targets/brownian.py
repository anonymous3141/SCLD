...
class Brownian(Target):
    def __init__(
        self, dim=32, log_Z=None, can_sample=False, sample_bounds=None
    ) -> None:
        ...
        self.target = gym.targets.VectorModel(
            target, flatten_sample_transformations=True
        )
    ...
    def log_prob(self, z: chex.Array):
        ...
        return self.target.unnormalized_log_prob(
            x
        ) + self.target.default_event_space_bijector.forward_log_det_jacobian(
            z, event_ndims=1
        )
